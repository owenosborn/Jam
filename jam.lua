...
function jam:init(io)
    ...
    self.player = ChordPlayer.new(nil, 5)  -- octave 4, chord will be set dynamically
    --self.player:setStyle("roll", {delay = io.dur(1/4)})
    self.player:setStyle("strum", {delay = io.dur(1/4), reverse=true})
    --self.player:setStyle("random", {window = io.dur(1)})
    self.count = 1    
    self.count2 = 1
    self.ccount = 1
...
function jam:tick(io)
    ...
    if io.on(1) then
        self.player:play(40, io.dur(1/4))  
        self.count2 = math.random(1, 4)
    end
   
    if io.on(1/2) then
        local n = chord_now:note((self.ccount % #chord_now.pitches) + 1, 5)--math.random(5,6)) 
        io.playNote(n, 60, io.dur(1/3))
        self.ccount = self.ccount + 1
    end

    if io.on(1/self.count2) and math.random() < 0.9 then 
        local n = chord_now:note(math.random(1, #chord_now.pitches), math.random(5,6)) 
        io.playNote(n, 60, io.dur(1/self.count2/2))
    end
