...
function jam:tick(io)
    ...
    if current_section == 0 then
        ...
        if io.on(1) then io.playNote(notes[1], 80, io.dur(1,4)) end
        if io.on(2) then io.playNote(notes[2], 80, io.dur(1,2)) end
        if io.on(2) then io.playNote(notes[3], 80, io.dur(1)) end
        if io.on(1) then io.playNote(notes[4], 80, io.dur(2)) end
        
    elseif current_section == 1 then
        -- Section B: Faster polyrhythm
        if io.on(1,2) then io.playNote(60, 70, io.dur(1,8)) end  -- Eighth notes
        if io.on(2,3) then io.playNote(67, 80, io.dur(1,4)) end
        if io.on(2) then io.playNote(72, 90, io.dur(1,2)) end
    ...
    elseif current_section == 2 then
        ...
        if io.on(1,9) then io.playNote(48, 60, io.dur(1,16)) end  -- crazy fast
        if io.on(1,6) then io.playNote(65, 70, io.dur(1,8)) end
